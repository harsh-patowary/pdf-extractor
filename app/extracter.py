...
# testing update 1
    ...
    def parse_pdf(path: Path) -> dict:
        # LOGGER.info(f"Parsing {path.relative_to(self.ROOT_DIR)}")

        # prepare data to be returned
        data = {
            "person": path.parent.name,
            "category": str(path.relative_to(self.ROOT_DIR).parts[0]),
            "file": path.name,
        ...
        try:
            data["email"] = self.extract_email(path)
            data["phone"] = self.extract_phonenum(path)
        except PyPDF2.utils.PdfReadError as e:
            # typically due to PDF being encrypted/locked
            self.logger.error(f"Failed to open {path.name}: {e}")
        except Exception as e:
            # don't want misc errors crashing the entire script
            # better to have a few blank emails
            self.logger.error(f"Failed to parse {path.name}: {e}")
    ...
    def validate_phonenum(text: str) -> Optional[str]:
        results = set()
        ph_number = set(re.findall(
            r'[\+\(]?[1-9][0-9 .\-\(\)]{8,}[0-9]', text))
        ...
        try:
            ...
            for field_key, field_text in fields.items():
                if 'email' in field_key.lower():
                    email = self.validate_email_string(field_text)
                    if email:
                        return email
        except TypeError:
            pass
    ...
    def get_email_from_pages(reader: PyPDF2.PdfFileReader) -> Optional[str]:
        ...
        for page in reader.pages:
            ...
            email = self.validate_email_string(text)
        ...
        try:
            ...
            for field_key, field_text in fields.items():
                if 'phone' in field_key.lower():
                    phone = self.validate_phonenum(field_text)
                    if phone:
                        return phone
        except TypeError:
            pass
    ...
    def get_phonenum_pages(reader: PyPDF2.PdfFileReader) -> Optional[str]:
        for page in reader.pages:
            text = page.extractText()
            phonenum = self.validate_phonenum(text)
    ...
    def extract_email(path: Path) -> Optional[str]:
        with open(path, "rb") as pdf:
            ...
            email = self.get_email_from_form(reader)
            if not email:
                email = self.get_email_from_pages(reader)
            if email:
                self.logger.info(
                    f"Emails found in {path.relative_to(self.ROOT_DIR)}: {email}")
                return email

    def extract_phonenum(path: Path) -> Optional[str]:
        with open(path, "rb") as pdf:
            reader = PyPDF2.PdfFileReader(pdf, strict=False)
            phone_number = self.get_phonenum_form(reader)
            if not phone_number:
                phone_number = self.get_phonenum_pages(reader)
            if phone_number:
                self.logger.info(
                    f"Phone Numbers found in {path.relative_to(self.ROOT_DIR)}: {phone_number}")
                return phone_number
...
# if __name__=='__main__':
    ...
    # glob_version = "*.pdf"
    # pdf_path = self.ROOT_DIR.rglob(glob_version)
    # with Pool() as p:
